				...
				case フェーズ.クローズ:
					{
						if( this._初めての進行描画 )
						{
							this._画面BC_アイキャッチ遷移画面1_回転中.ぼかしを適用する( gd );
							this._画面BC_アイキャッチ遷移画面1_回転中.縮小を適用する( gd );
				...
				case フェーズ.オープン:
					{
						if( this._初めての進行描画 )
						{
							this._画面BC_アイキャッチ遷移画面1_回転中.ぼかしを適用する( gd, 0.0 );      // 0.0秒以内 → 最初から全部適用状態。
							this._画面BC_アイキャッチ遷移画面1_回転中.縮小を適用する( gd, 0.0 );

							this._画面D_アイキャッチ遷移画面2_逆回転中.ぼかしを適用する( gd, 0.0 );	// 全部適用してから
							this._画面D_アイキャッチ遷移画面2_逆回転中.縮小を適用する( gd, 0.0 );
							this._画面D_アイキャッチ遷移画面2_逆回転中.ぼかしを解除する( gd );         // ゆっくり解除開始。
							this._画面D_アイキャッチ遷移画面2_逆回転中.縮小を解除する( gd );
