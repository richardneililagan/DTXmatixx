...
namespace DTXmatixx.ステージ.認証
{
	class 認証ステージ : ステージ
	{
		...
		public 認証ステージ()
		{
			...
			this.子リスト.Add( this._フェードアウト = new 回転幕() );
		}
		...
		public override void 進行描画する( グラフィックデバイス gd )
		{
			...
			switch( this.現在のフェーズ )
			{
				case フェーズ.フェードイン:
					...
					if( this._フェードイン.現在のフェーズ == シャッター.フェーズ.オープン完了 )
					{
				...
				case フェーズ.フェードアウト:
					this._舞台画像.進行描画する( gd, ( 2250 >= this._表示フェーズカウンタ.現在値 ) );
					this._フェードアウト.進行描画する( gd );

					if( this._フェードアウト.現在のフェーズ == 回転幕.フェーズ.クローズ完了 )
					{
						this._待機フェーズカウンタ = new Counter( 0, 2000, 1 );   // 全2秒
						this.現在のフェーズ = フェーズ.待機;
					}
					break;
